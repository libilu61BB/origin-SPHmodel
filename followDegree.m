...
%     end
    ...
    %% 统计稳定运行后行人轨迹
    % 运行时间50s之后，取x=20~30区域内的200个粒子轨迹进行统计
    if t > 50 && length(trace2r_index)<100 % 统计到达x=20处并向右走的粒子
...
%         end
    ...
    if t > 50 && length(trace2l_index)<100 % 统计到达x=30处并向左走的粒子
...
%         end
    ...
    % 轨迹数>100且各粒子都走过了统计区域之后
    if length(trace2l_index)>=100 && length(trace2r_index)>=100
        last2l_index = trace2l_index(end);
        last2r_index = trace2r_index(end);
        if person_x(last2l_index)<20 && person_x(last2r_index)>30
            break;
        end
    end
...
xlswrite('2rx_tolerance_1_Kfoll_10.xlsx', trace2r_x);
xlswrite('2ry_tolerance_1_Kfoll_10.xlsx', trace2r_y);
xlswrite('2lx_tolerance_1_Kfoll_10.xlsx', trace2l_x);
xlswrite('2ly_tolerance_1_Kfoll_10.xlsx', trace2l_y);
