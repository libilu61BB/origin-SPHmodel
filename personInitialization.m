...
%% 欸欸这个是验证文献左右两边双向流实验的初始化
person_x = [];
person_y = [];
while length(person_x) < 200
    flag = 1;
    temp_x = -1.2-4.6*rand;
    temp_y = -1.2+4.6*rand;
    for i=1:length(person_x)
        r = sqrt((temp_x-person_x(i))^2 + (temp_y-person_y(i))^2);
        if r<0.4
            flag=0;
            break;
        end
    ...
    if flag == 1
        person_x = [person_x temp_x];
        person_y = [person_y temp_y];
    end
end